"""v47: v44 + smart backend selection (HF when it fits well, cv2 fallback).

Halir-Flusser gives sub-pixel precision on near-ellipses but can return
oscillating fits on pure circles (numerical degeneracy). Strategy:
  1. Try HF refit
  2. If HF residual > 1.5 px AND cv2 fitEllipse succeeds: use cv2
  3. If both succeed, prefer HF (sub-pixel)
  4. If only one succeeds, use that one

This eliminates v44's false-positive explosion on near-circular wire
turns (038_009: 4→4 instead of 4→0) while keeping v44's sub-pixel a/b
advantage on truly elliptical fits.
"""
...
def _ellipse_residual_px(pts, a, b, ang_deg, cx, cy):
    if len(pts) < 5 or a <= 0 or b <= 0:
        return 999.0
    ar = np.radians(ang_deg)
    cos_a, sin_a = np.cos(ar), np.sin(ar)
    x = pts[:, 0] - cx
    y = pts[:, 1] - cy
    x_loc = cos_a * x + sin_a * y
    y_loc = -sin_a * x + cos_a * y
    R_prime = np.sqrt((x_loc / a) ** 2 + (y_loc / b) ** 2)
    return float(np.abs(R_prime - 1.0).mean() * min(a, b))


def _fit_best(points_xy, *, residual_thr=1.5):
    """Return (cx, cy, a, b, ang_deg, src, residual) — best of HF vs cv2."""
    hf_res = hf.halir_flusser_fit(points_xy)
    cv2_res = v32._fit_ellipse_safe(points_xy.reshape(-1, 1, 2))
    candidates = []
    if hf_res is not None:
        cx, cy, a, b, ang = hf_res
        res = _ellipse_residual_px(points_xy, a, b, ang, cx, cy)
        candidates.append({"cx": cx, "cy": cy, "a": a, "b": b,
                           "ang": ang, "residual": res, "src": "hf"})
    if cv2_res is not None:
        cx, cy, a, b, ang = cv2_res
        res = _ellipse_residual_px(points_xy, a, b, ang, cx, cy)
        candidates.append({"cx": cx, "cy": cy, "a": a, "b": b,
                           "ang": ang, "residual": res, "src": "cv2"})
    if not candidates:
        return None
    if len(candidates) == 1:
        return candidates[0]
    # Both succeed: prefer HF IF residual < threshold
    hf_c = next(c for c in candidates if c["src"] == "hf")
    cv2_c = next(c for c in candidates if c["src"] == "cv2")
    if hf_c["residual"] <= residual_thr:
        return hf_c
    return cv2_c


def fit_v47(mask_bin, *,
    ...
    for round_idx in range(n_rounds):
        ...
        if new_mask.sum() < min_support:
            continue
        best = _fit_best(free_pixels[new_mask], residual_thr=residual_thr)
        if best is None:
            continue
        cx = best["cx"]; cy = best["cy"]
        a = best["a"]; b = best["b"]; ang_deg = best["ang"]
        ...
        seeds.append({
            ...
            "a": float(a), "b": float(b),
            "r": float(r_ell),
            "ang": float(ang_deg), "ratio": float(ratio),
            "support": int(score),
            "sigma": float(sigma),
