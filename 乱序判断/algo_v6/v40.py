"""v40: v39 production + cupy ZS84 GPU skeleton.

Hypothesis: replace skimage Lee (130ms) with cupy Zhang-Suen (27ms) for
~100ms/100ms = 100% potential speedup. Verify topology impacts downstream N.

Pipeline same as v39 except:
  v32._skeleton(mask_bin) → skel_cupy.zhang_suen_cupy(mask_bin)
"""
...
def fit_v40(mask_bin, *,
    ...
    skel_pix, _ = skel_cupy.zhang_suen_cupy(mask_bin)
    if len(skel_pix) < 30:
        return {"circles": [], "N": 0, "elapsed_s": {"total": time.time() - t0}}
    t1 = time.time()
