"""v46: v44 + post-refit residual check (rejects false-positive HF fits).

After fitting ellipse, compute mean |R'(theta) - 1| where R'=ellipse_radius
parameterisation of (x/a)^2 + (y/b)^2 under rotation. A perfect ellipse fit
gives R'=1 everywhere → residual=0. Points that lie on a CIRCLE not an
ellipse give residual > 0 and are rejected.
"""
...
def _ellipse_residual_px(pts: np.ndarray, a: float, b: float,
                         ang_deg: float, cx: float, cy: float) -> float:
    """Mean |R'-1| * min(a,b). For perfect ellipse → 0."""
...
def _fit_ellipse_hf(points_xy):
    ...
    if res is None:
        return None
    cx, cy, a, b, ang_deg = res
    return {"cx": cx, "cy": cy, "a": a, "b": b, "ang": ang_deg}


def fit_v46(mask_bin, *,
    ...
    for round_idx in range(n_rounds):
        ...
        if new_mask.sum() < min_support:
            continue
        hf_p = _fit_ellipse_hf(free_pixels[new_mask])
        if hf_p is None:
            continue
        cx, cy, a, b, ang_deg = (hf_p["cx"], hf_p["cy"], hf_p["a"],
                                 hf_p["b"], hf_p["ang"])
        ...
        if (cx < cx_lo2 or cx > cx_hi2 or
            ...
            continue
        residual = _ellipse_residual_px(
            free_pixels[new_mask], a, b, ang_deg, cx, cy)
