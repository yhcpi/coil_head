"""v50: v39 + downsample mask 2x before pipeline, upsample result.

Skeleton cost ~ H*W pixels. Halving dimensions (1/4 pixels) should cut
skel + RANSAC distance matrix by 3-4x. After fitting, scale (cx, cy, a, b, r)
back by 2.

Sub-pixel RANSAC on coarse mask requires:
  - Sub-pixel edge accuracy lost → wire turns may smear into wider rings
  - Band_px scales with ds → must halve from 5 to 2.5 px in coarse space
  - min_support scales with perimeter → must quarter from 400 to 100

This is a "compute cheap, then post-correct" approach.
"""
...
def fit_v50(mask_bin, *, scale=2,
            n_rounds=12, n_iter=200,
            band_px=2.5, min_support=40,  # scale-dependent defaults
            r_min_factor=0.10, r_max_factor=0.95,
            ratio_min=1.00, ratio_max=1.10,
            support_min=80, dr_tol_px=2.5,
    ...
    if scale == 1:
        ...
        m_down = cv2.resize(mask_bin, (W_orig // scale, H_orig // scale),
                            interpolation=cv2.INTER_AREA)
        m_down = (m_down > 127).astype(np.uint8)
    H, W = m_down.shape[:2]
    skel, skel_pixels = v32._skeleton(m_down)
    ...
    for round_idx in range(n_rounds):
        ...
        seeds.append({
            "cx": float(cx * scale), "cy": float(cy * scale),
            "a": float(a * scale), "b": float(b * scale),
            "r": float(r_ell * scale),
