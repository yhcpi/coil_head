"""v51: v39 + bounding-box crop + coordinate offset.

Average mask fill ratio is 62% (100-image survey). Cropping to the
mask's actual content + 30px padding runs skeleton + RANSAC on
~40% smaller image. After fit, add (x0, y0) offset to (cx, cy).

Boundary check `cx < cx_lo2 or cx > cx_hi2` (originally [-W*0.05, W*1.05])
must be relative to the CROPPED image — easy since we compute H, W
from cropped mask.
"""
