"""v49: v39 + cupy ZS84 + connected-component length-8 filter.

Skips skimage Lee (CPU 65ms) in favour of cupy ZS84 (GPU 26ms isolated) +
post-filter to remove zigzag artifacts. Filter: only keep skeleton pixels
in connected components of size >= 8 (RANSAC needs ~30+ points on arc,
short zigzag segments < 8 can't support a circle).
"""
...
import v35
import v38
import skel_cupy_v49 as skel_v49

PALETTE = v32.PALETTE


def fit_v49(mask_bin, *,
    ...
    skel_xy = skel_v49.zs_skeletonize(mask_bin)
    skel_xy = skel_v49.filter_long_branches(skel_xy, H, W,
                                            min_size=min_branch_size)
    if len(skel_xy) < 30:
        return {"circles": [], "N": 0,
    ...
    seeds = []
    claimed_mask = np.zeros(len(skel_xy), dtype=bool)
    for round_idx in range(n_rounds):
        free_pixels = skel_xy[~claimed_mask]
