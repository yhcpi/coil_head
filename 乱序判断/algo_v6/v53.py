"""v53: v39 + cv2.distanceTransform skeleton (medial axis via DT).

Compute distance from each foreground pixel to nearest background pixel.
Keep pixels where DT > dt_threshold as "medial axis" (rough skeleton).

This is not a true 1-pixel skeleton but gives axis positions which is
what 3-pt RANSAC needs (arc support). Faster than Lee when mask has
substantial solid regions because DT is OpenCV's fast algorithm.

dt_threshold ~ 3-5 px gives ~wire-diameter-thick medial axis.
"""
...
def _dt_skeleton(mask_bin: np.ndarray, dt_threshold: float = 3.0) -> np.ndarray:
    """Distance-transform-based medial axis. Returns (x, y) int32 array."""
    m8 = (mask_bin > 0).astype(np.uint8) * 255
    dist = cv2.distanceTransform(m8, cv2.DIST_L2, 5)
    skel = (dist >= dt_threshold).astype(np.uint8)
...
def fit_v53(mask_bin, *,
    ...
    skel_pixels = _dt_skeleton(mask_bin, dt_threshold=dt_threshold)
