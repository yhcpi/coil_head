"""v39: combination of v37 (modest RANSAC budget) + v38 (support/r adaptive).

Changes from v35 baseline:
  - n_rounds 12 → 12 (kept), n_iter 300 → 200 (modest cut)
  - arc_min hard 0.20 → support/r adaptive [0.45, 0.15]

Expected:
  - t_med: 121ms → 105-110ms (modest speedup from n_iter cut)
  - N: 7 → 5 (v38 strict precision, lost 2 wires median on 100img eval)
  - Gold standard 017_roi_004: v35 N=6 → v39 N=3 (matches GT=3)
"""
...
def fit_v39(mask_bin, *,
    ...
    for round_idx in range(n_rounds):
        ...
        if (cx < cx_lo2 or cx > cx_hi2 or
            ...
            continue
        # v38: support/r adaptive arc_min
...
def render(raw_bgr, mask_bin, result, out_path, gt_path=None):
    """v39 render: combines v37 + v38 features."""
    p = raw_bgr.copy()
    overlay = p.copy()
    ...
    for e in result["circles"]:
        ...
        sr_str = f" sr{e.get('support_per_r', 0):.1f}"
        cv2.putText(p,
    ...
    cv2.putText(p,
                f"v39: N={result['N']}({n_warn}!)  t={total_s*1000:.0f}ms  "
                f"R={result.get('n_rounds', 0)}x{result.get('n_iter', 0)}",
                (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                (255, 255, 255), 2)
    cv2.putText(p,
                f"v35 + support/r arc + n_iter=200  a/b<={result['ratio_range'][1]}",
...
def main():
    ...
    pairs = [
        ("yhc/mask_refine/038_roi_009.png", "v39_038_roi_009.png"),
        ("yhc/mask_refine/038_roi_001.png", "v39_038_roi_001.png"),
        ("yhc/mask_refine/005_roi_001.png", "v39_005_roi_001.png"),
        ("yhc/mask_refine/017_roi_001.png", "v39_017_roi_001.png"),
        ("yhc/mask_refine/017_roi_004.png", "v39_017_roi_004.png"),
    ]
    if len(sys.argv) > 1:
        frag = sys.argv[1]
    ...
    out = project / "algo_v6" / "v39_summary.json"
