"""v45: v44 + adaptive RANSAC round budget.

Stop RANSAC early to save time:
  - Stop after `max_accepted` seeds are accepted (default 8)
  - Stop after `patience` consecutive rounds that fail the chain of
    σ>σ_max → refit fail → ratio out → bounds → arc_min filters
    (default patience=5)

Same threshold chain as v44 (HF refit + ratio + bounds + arc_min).
"""
...
def fit_v45(mask_bin, *,
            n_rounds=12, n_iter=200,
            max_accepted=8, patience=5,
    ...
    for round_idx in range(n_rounds):
        ...
        if sigma > sigma_max:
            no_seed_count += 1
            if no_seed_count >= patience: break
            continue
        ...
        if arc_pixels < arc_min:
            ...
            if no_seed_count >= patience: break
            continue
        no_seed_count = 0  # reset on accept
        seeds.append({
    ...
    return {
        ...
        "max_accepted": max_accepted, "patience": patience,
        "rounds_used": round_idx + 1 if seeds else 0,
        "elapsed_s": {
