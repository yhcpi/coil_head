...
# 物理感知增强（PA-Aug）：钢卷头尾检测特化的 4 个独立组件
from .pa_aug import make_paaug
...
class Format:
    ...
    def __call__(self, labels):
        ...
        _b = instances.bboxes
        labels['bboxes'] = _b if torch.is_tensor(_b) else torch.from_numpy(_b)
        if nl == 0:
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    # MosaicNeg patch: 如果用户启用了 MosaicNeg，则替换 Mosaic
    from .mosaic_neg import MosaicNeg, get_neg_params
    neg_p, neg_min, neg_max = get_neg_params()
    if neg_p > 0:
        mosaic_op = MosaicNeg(dataset, imgsz=imgsz, p=hyp.mosaic, n=4,
                              neg_p=neg_p, neg_min=neg_min, neg_max=neg_max)
    else:
        mosaic_op = Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic)
    ...
    transforms = [
        ...
        # Coil-specific hook: bbox_random_shrink (默认通过 hyp.bbox_shrink_min/max 控制；1.0/1.0 = 关闭)
        BBoxRandomShrink(
            scale_min=getattr(hyp, 'bbox_shrink_min', 1.0),
            scale_max=getattr(hyp, 'bbox_shrink_max', 1.0),
            p=getattr(hyp, 'bbox_shrink_p', 1.0)),
        # Coil-specific hook: multi-scale rect (默认通过 hyp.multi_scale 控制；0 = 关闭)
        RandomScaleRect(scale_range=getattr(hyp, 'multi_scale', 0.0)),
    ]  # noqa: E501
    # PA-Aug: 物理感知增强（钢卷场景特化）— 4 组件可单选；paaug=none 时为 None（过滤掉，等价 v4）
...
# =============================================================================
# Coil-specific augmentation: BBoxRandomShrink
# =============================================================================
# 创新点：把每个 GT bbox 的宽高按 0.8~1.2 倍随机缩放，**保持中心点不变**，
# 生成额外的"伪 GT"作为训练信号。这等价于笔记 §0.3 中"概率性 bbox 建模"的轻量版。
#
# 设计动机：
#   - 钢卷头部 GT 是"宽容标注"（bbox 包住目标即可）
#   - 让模型学到"目标在 GT 范围内的任何位置都行"——是 Coverage Loss 的近似实现
#   - 不需要替换 loss，只在数据加载时增加样本多样性
#
# 启用方法（hyp_aug.yaml）：
#   bbox_shrink_min: 0.8   # 边长最小缩放
#   bbox_shrink_max: 1.2   # 边长最大缩放
#   bbox_shrink_p: 1.0     # 应用概率（1.0 = 每张图都做）
#
# 默认值（hyp_aug.yaml 中写 1.0/1.0 或 0）：1.0 = 启用，<=1.0 = 关闭
# =============================================================================


class BBoxRandomShrink:
    """对每个 GT bbox 的宽高做随机缩放（保持中心），用于宽容标注场景。

    不依赖 albumentations，独立实现。
    """
    def __init__(self, scale_min=0.8, scale_max=1.2, p=1.0):
        self.scale_min = float(scale_min)
        self.scale_max = float(scale_max)
        self.p = float(p)

    def __call__(self, labels):
        """labels: dict 含 'instances' (含 bboxes)。"""
        import random
        ...
        if 'instances' not in labels or len(labels['instances']) == 0:
            return labels
        bboxes = labels['instances'].bboxes  # (N, 4) xyxy 像素 (numpy 或 Tensor)
        if len(bboxes) == 0:
            return labels
        # 对每个 bbox 独立随机缩放宽高，保持中心
        ...
        # Bypass Instances.bboxes read-only property by mutating the underlying Bboxes
        # object directly. We also need to mirror the update back to segments / keypoints
        # if they exist (they don't for our task, but keep safe).
        inst = labels['instances']
        inst._bboxes.bboxes = new_bboxes
        if hasattr(inst, 'segments') and len(inst.segments):
            inst.segments = []
        labels['instances'].segments = []  # 兼容 segments 字段
        return labels

    def __repr__(self):
        return f'BBoxRandomShrink(scale_min={self.scale_min}, scale_max={self.scale_max}, p={self.p})'


class RandomScaleRect:
    """在 rect 训练下做多尺度缩放，**保持长宽比不变**。

    与 ultralytics 标准 multi-scale 的区别：
      - 标准 multi-scale 缩放 imgsz（可能破坏长宽比）
      - RandomScaleRect 缩放 imgsz，但按比例同步缩放 w 和 h → 长宽比不变

    启用方法（hyp_aug.yaml）：
      multi_scale: 0.2   # ±20%，即 imgsz 在 [820, 1228] 之间随机
      （0 = 关闭）
    """
    def __init__(self, scale_range=0.2):
    ...
    def __call__(self, labels):
        ...
        if 'img' in labels:
            ...
            if hasattr(img, 'shape'):
                ...
                labels['img'] = img
        # 同步缩放所有 bbox（xyxy 像素 → 按 scale 缩放）
        if 'instances' in labels and len(labels['instances']) > 0:
            bboxes = labels['instances'].bboxes
            if len(bboxes) > 0:
                # Bypass Instances.bboxes read-only property by mutating internal Bboxes
                if not torch.is_tensor(bboxes):
                    bboxes = torch.as_tensor(bboxes, dtype=torch.float32)
                labels['instances']._bboxes.bboxes = bboxes * scale
        return labels
