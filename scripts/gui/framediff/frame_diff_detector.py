...
# [v8] 参考帧率 (3.mp4 = 60fps). 所有时间相关参数 (deque maxlen, 检查点, 窗口)
# 在 update() 时按 self.fps / REFERENCE_FPS 归一化, 30fps 下窗口自动减半.
REFERENCE_FPS = 60
...
class FrameDiffCoilDetector:
    def __init__(
        ...
        # [v8] fps 归一化: update() 时按入参覆盖, 默认 REFERENCE_FPS (60)
        # 所有时间窗/检查点/衰减都按 self.fps 缩放, 30fps 行为自动减半
        self.fps = REFERENCE_FPS
        self._max_decay_per_frame = 0.999      # 60fps 等价, 按 (60/fps) 指数缩放
        self._baseline_decay_per_frame = 0.9999  # 同上
        ...
        # [v8] deque maxlen 按 fps 缩放: 60fps=60 帧 (1s), 30fps=30 帧
        _fps0 = REFERENCE_FPS  # __init__ 时 fps 尚未确定, 用 REFERENCE_FPS 占位
        self.filled_history = deque(maxlen=max(rise_window, fall_window, saturation_window, int(1.0 * _fps0)))
        # 辅助指标 hull_area - 凸包面积
        self.hull_history = deque(maxlen=int(1.0 * _fps0))
        # solidity_history - contour / hull
        self.solidity_history = deque(maxlen=int(0.5 * _fps0))
        ...
        self._long_history = deque(maxlen=int(1.0 * _fps0))
        # 饱和期内峰值参考: 最近 60 帧 smoothed 最大值
        self.sat_peak_history = deque(maxlen=int(1.0 * _fps0))
        ...
        # [v8] maxlen 按 fps 缩放: 60fps=300 帧 (5s), 30fps=150 帧
        self.consec_history = deque(maxlen=int(5.0 * _fps0))
        self._consec_peak = 0.0  # rolling max of consec_history, settled 期更新
        # [v8] 滚动峰值/单调性窗口按 fps 缩放 (60fps 等价原值)
        self._consec_peak_window_frames = int(round(consec_peak_window_sec * _fps0))
        self._consec_mono_window_frames = int(round(consec_mono_window_sec * _fps0))
    ...
    def _is_falling_trend_only(self, filled):
        ...
        if self._coil_peak > 0 and filled > self._coil_peak * self.fall_peak_ratio:
            return False
        # [v8] 窗口按 fps 缩放: 60fps=30 帧 (0.5s), 30fps=15 帧
        _win_half = int(0.5 * self.fps)
        if self._filled_monotone_count(_win_half) < self.fall_monotone_min:
            return False
        if self._hull_monotone_count(_win_half) < self.fall_hull_monotone_min:
            return False
        return True
    ...
    def _is_hull_shrinking(self):
        ...
        if self._stable_coil_frames < self.settle_guard_frames:
            return False
        # [v8] 窗口按 fps 缩放: 60fps=30 帧, 30fps=15 帧
        _win_half = int(0.5 * self.fps)
        if len(self.hull_history) < _win_half:
            return False
        ...
        if drop_ratio < 0.05:
            return False
        m_h = self._hull_monotone_count(_win_half)
        # [v8] monotone 阈值按 fps 缩放: 60fps=20 (~67%), 30fps=10
        if m_h < int(0.67 * _win_half):
            return False
        return True
    ...
    def _is_falling(self, filled):
        ...
        if result and not getattr(self, '_dbg_falling_printed', False):
            print(f'  [is_falling TRUE @ F{self.frame_idx} t={self.frame_idx/self.fps:.2f} filled={filled:.0f} peak={self._coil_peak:.0f}]')
    ...
    def _is_unsaturating(self):
        ...
        if result and not getattr(self, '_dbg_unsat_printed', False):
            print(f'  [is_unsaturating TRUE @ F{self.frame_idx} t={self.frame_idx/self.fps:.2f} peak={self._coil_peak:.0f}]')
    ...
    def _is_unsaturating_impl(self):
        ...
        [v8] 窗口与 monotone 阈值按 fps 归一化: 60fps=30 帧/25 (~83%),
        30fps=15 帧/12 (~80%). 比例语义保持一致.
        """
        if not self.entered or self._coil_peak <= 0:
            return False
        # [v8] 窗口按 fps 缩放
        _win_half = int(0.5 * self.fps)
        if len(self.filled_history) < _win_half:
            return False
        recent = list(self.filled_history)[-_win_half:]
        median_recent = float(np.median(np.array(recent, dtype=np.float32)))
        if median_recent >= self._coil_peak * self.unsaturating_ratio:
            return False
        # [v8] 比例归一化: 旧默认 25 (60fps 30 帧中 ~83%) → 比例 = 25/29 ≈ 0.862
        # 60fps: win=30, ratio=0.862, mono ≥ 24 (vs 原 25, 差 1 帧可接受)
        # 30fps: win=15, mono ≥ 12 (~80%)
        _ref_win = int(0.5 * REFERENCE_FPS)  # 30
        mono = sum(1 for i in range(_win_half - 1) if recent[i] > recent[i + 1])
        ratio_thresh = self.unsaturating_mono_min * (_win_half - 1) / _ref_win
        return mono >= int(ratio_thresh)
    ...
    def _is_consec_settled(self):
        ...
        判定条件:
        - consec median of last N frames < peak × 0.05  (≈ 1.5-2 万)
        - consec median of last M frames < peak × 0.03  (≈ 1 万, 紧约束)
        - consec 连续 K 帧 ≤ peak × 0.05  (无反弹, 用 max_run 统计)

        [v8] 窗口按 fps 归一化: 60fps N=30/M=10/K=20, 30fps N=15/M=5/K=10.
        """
        if not self._consec_peak_ready or self._consec_peak <= 0:
            return False
        if not self._has_emitted_leave:
            return False
        # [v8] 窗口按 fps 缩放
        _win_half = int(0.5 * self.fps)        # 60fps=30, 30fps=15
        _win_tight = int(0.17 * self.fps)      # 60fps=10, 30fps=5
        _run_thresh = int(0.67 * _win_half)    # 60fps=20, 30fps=10
        if len(self.consec_history) < _win_half:
            return False
        recent_n = list(self.consec_history)[-_win_half:]
        med_n = float(np.median(np.array(recent_n, dtype=np.float32)))
        recent_m = list(self.consec_history)[-_win_tight:]
        med_m = float(np.median(np.array(recent_m, dtype=np.float32)))
        # 紧约束: median 必须持续低于 peak 的 5% / 3%
        if med_n >= self._consec_peak * 0.05:
            return False
        if med_m >= self._consec_peak * 0.03:
            return False
        # [v8] 用 max_run 显式统计连续低值帧 (原 consec_settle_count 遇 false 归零,
        # 30fps 缩放时与归一化语义不清晰)
    ...
    def _update_state_machine(self, smoothed_area):
        ...
        if self.event_log and self.event_log[-1][1] == 'LEAVE' and self.frame_idx - self.event_log[-1][0] <= 2:
            print(f'  [LEAVE FIRE @ F{self.frame_idx} t={self.frame_idx/self.fps:.2f} state={self.state} sub={self.sub_state} filled={smoothed_area:.0f} peak={self._coil_peak:.0f}]')
        ...
        if self._has_emitted_leave and not self.entered:
            ...
            # [v8] 2.5s 后 (按 fps 缩放: 60fps=150 帧, 30fps=75 帧) 双信号联合检查
            self._post_leave_check_count = getattr(self, '_post_leave_check_count', 0) + 1
            if self._post_leave_filled_at_leave > 0 and self._post_leave_check_count == int(2.5 * self.fps):
                ...
                if is_false_leave:
                    print(f'  [FALSE LEAVE RECOVER @ F{self.frame_idx} t={self.frame_idx/self.fps:.2f} '
        ...
        # max_seen 滚动跟踪: max(max_seen × decay, filled)
        # [v8] decay 按 fps 缩放: 60fps=0.999, 30fps=0.99801 (衰得更快)
        if filled_smoothed > 1.0:
            self.max_seen = max(self.max_seen * self._max_decay_per_frame, filled_smoothed)

        # baseline: coil 稳定时的 filled 参考
        # [v8] baseline decay 按 fps 缩放: 60fps=0.9999, 30fps=0.9998
        if self.entered and self.sub_state == 'STABLE_COIL' and filled_smoothed > 1.0:
            if not hasattr(self, '_baseline') or self._baseline < filled_smoothed:
                ...
                self._baseline = max(self._baseline * self._baseline_decay_per_frame, filled_smoothed)

        # Warmup 保护
        # [v8] warmup 1s 预填按 fps 缩放: 60fps=60 帧, 30fps=30 帧
        warmup_end = self.bg_frames + int(1.0 * self.fps)
        if self.frame_idx < warmup_end:
            ...
            self.above_enter_count = 0
            self._below_enter_count = 0
            for _ in range(int(1.0 * self.fps)):
        ...
        if (self._has_emitted_leave
            ...
            # [v8] 1s 持续按 fps 缩放: 60fps=60 帧, 30fps=30 帧
            if self._below_enter_count > int(1.0 * self.fps):
                raw_new_sub = 'STABLE_NO_COIL'
                self._fall_sticky_active = False
            else:
    ...
    def update(self, frame_bgr, fps=REFERENCE_FPS):
        gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
        gray = cv2.GaussianBlur(gray, (5, 5), 0)
        self.frame_idx += 1
        # [v8] fps 存值 + 保护 (10-120, 兜底 REFERENCE_FPS)
        # 所有"按 60fps 调优的硬编码数字"用 self.fps 归一化, 30fps 行为自动减半
        self.fps = max(10, min(fps if fps and fps > 0 else REFERENCE_FPS, 120))
        # 每帧衰减按 fps 缩放: 60fps 时 0.999 保持不变
        self._max_decay_per_frame = 0.999 ** (REFERENCE_FPS / self.fps)
        self._baseline_decay_per_frame = 0.9999 ** (REFERENCE_FPS / self.fps)
        ...
        # [v8-fix] 用 hull_history 最近 3s 窗口的 percentile(75) 而非单点 max
        # - 单点 max 会被单帧尖峰污染
        # - 1s median 在 30fps 只有 30 帧, coil settling 期 hull 大幅波动时
        #   1s 窗口内 hull 持续下跌, 拉低 median 永久污染 reference
        #   (5.mp4 hull_settled_peak=335k 但实际 STABLE 持续峰值 ~600k+)
        # - 3s percentile(75) 提供鲁棒高估, 抗短时波动与单帧尖峰
        #   60fps=180 帧, 30fps=90 帧, 与 consec_peak 的 2s 滚动窗口思路一致
        if (self.entered and self.sub_state == 'STABLE_COIL'
                and self._stable_coil_frames > self.settle_guard_frames
                and len(self.hull_history) >= int(3.0 * self.fps)):
            _hull_window = np.array(list(self.hull_history)[-int(3.0 * self.fps):],
                                    dtype=np.float32)
            _hull_p75 = float(np.percentile(_hull_window, 75))
            if _hull_p75 > self._hull_settled_peak:
                self._hull_settled_peak = _hull_p75
        ...
        # [v8] 3s 缓冲按 fps 缩放: 60fps=180 帧, 30fps=90 帧
        if (self.entered and self.sub_state == 'STABLE_COIL'
                and self._stable_coil_frames > self.settle_guard_frames + int(3.0 * self.fps)
    ...
    def reset(self):
        ...
        # [v8] reset 时也重置 fps 与衰减系数 (下次 update() 会按新 fps 重算)
        self.fps = REFERENCE_FPS
        self._max_decay_per_frame = 0.999
        self._baseline_decay_per_frame = 0.9999
        ...
        # [v8] reset 重建 deque 时也按 fps 归一化 (与 __init__ 一致)
        self.area_history.clear()
        self.filled_history = deque(maxlen=max(self.rise_window, self.fall_window, self.saturation_window, int(1.0 * self.fps)))
        self.hull_history = deque(maxlen=int(1.0 * self.fps))
        self.solidity_history = deque(maxlen=int(0.5 * self.fps))
        self.smoothed_history.clear()
        self.sat_peak_history = deque(maxlen=int(1.0 * self.fps))
        self._long_history = deque(maxlen=int(1.0 * self.fps))
        self.above_enter_count = 0
        self._below_enter_count = 0
        self.max_seen = 1.0
        self.pending_state = None
        ...
        if hasattr(self, '_re_rising_count'):
            self._re_rising_count = 0
        # [v8] reset 重建 consec_history (与 __init__ 一致) 和重算窗口
        self.consec_history = deque(maxlen=int(5.0 * self.fps))
        self._consec_peak = 0.0
        self._consec_peak_window_frames = int(round(self.consec_peak_window_sec * self.fps))
        self._consec_mono_window_frames = int(round(self.consec_mono_window_sec * self.fps))
        self._consec_peak_ready = False
        self.event_count = {'ENTER': 0, 'LEAVE': 0}
