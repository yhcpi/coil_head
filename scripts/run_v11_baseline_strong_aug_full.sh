#!/bin/bash
# v11 baseline + 强 aug (degrees=10, copy_paste=0.2) 250 epoch full 重训
# 目的: 当前环境实际比历史 baseline 训练时更好 2.2x；测试强 aug 在新环境下是否仍退化
# 改动: degrees 0.0 → 10.0, copy_paste 0.0 → 0.2
# 其他 100% 复用 run_v8_nwd_v1_weak_aug_full.sh（已 100% 复现验证）
set -u
cd /home/pi/projects/hyperyolo

NAME="v11_baseline_strong_aug_full"

echo "===== 检查现有 ultralytics 进程 ====="
ps -ef | grep -v grep | grep "ultralytics" | head -5 || echo "  (无现有进程)"

echo "===== 启动 v11 baseline 强 aug (250 epoch) ====="
echo "改动: degrees 0.0 → 10.0, copy_paste 0.0 → 0.2（其他不变）"
