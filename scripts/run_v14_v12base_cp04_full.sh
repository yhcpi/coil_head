#!/bin/bash
# v14 = v12 baseline + 3 张真漏标 + copy_paste=0.4 + 250 epoch
# 改动 (相对 v12):
#   - data/coil/labels/train/{166,183,400}.txt 新增 1 个 GT（+3 正 / -3 负）
#   - copy_paste: 0.2 → 0.4（您想验证更强 aug 是否帮样本不足场景）
#   - epochs: 300 → 250
# 其他 100% 沿用 v12 baseline 配置
# 预训练起点: repos/Hyper-YOLO/hyper-yolon.pt（不沿用 v12 best.pt）
set -u
cd /home/pi/projects/hyperyolo

NAME="v14_v12base_cp04_full"

echo "===== 检查现有 ultralytics 进程 ====="
ps -ef | grep -v grep | grep "ultralytics" | head -5 || echo "  (无现有进程)"

echo "===== 启动 v14 (= v12 baseline + 3 张新 GT + copy_paste=0.4) 250 epoch ====="
echo "改动: +3 张真漏标 GT，copy_paste 0.2→0.4，epochs 300→250，其他 100% 沿用 v12"
/home/pi/anaconda3/envs/hyper-yolo/bin/python -m ultralytics.models.yolo.detect.train \
  ...
  mosaic=0.0 mixup=0.0 copy_paste=0.4 \
  multi_scale=0.0 \
