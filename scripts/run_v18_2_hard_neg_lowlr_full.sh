#!/bin/bash
# v18.2 = v12 baseline + 33 hard neg + 低学习率短训练（调优版）
# 改动 (相对 v18):
#   - lr0: 0.01 → 0.001 (10x 低，避免 fine-tune 剧烈震荡)
#   - epochs: 250 → 80 (短训练，让模型有时间稳定)
#   - patience: 80 → 30 (早停)
#   - 其他 100% 沿用 v18 (= v12 baseline + hard neg 增强)
set -u
cd /home/pi/projects/hyperyolo

NAME="v18_2_hard_neg_lowlr_full"

echo "===== 检查现有 ultralytics 进程 ====="
ps -ef | grep -v grep | grep "ultralytics" | head -5 || echo "  (无现有进程)"

echo "===== 启动 v18.2 (hard neg + lr=0.001 + 80ep) ====="
/home/pi/anaconda3/envs/hyper-yolo/bin/python -m ultralytics.models.yolo.detect.train \
  ...
  epochs=80 patience=30 batch=16 imgsz=1024 \
  ...
  fraction=1.0 \
  lr0=0.001 lrf=0.0001 momentum=0.937 weight_decay=0.0005 \
  warmup_epochs=3.0 warmup_momentum=0.8 warmup_bias_lr=0.0001 \
