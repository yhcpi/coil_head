#!/bin/bash
# D-3 smoke test v2：paaug=motion + coverage=false 30 epoch 验证修复
set -e
cd /home/pi/projects/hyperyolo
echo "==== D smoke test v2 (paaug=motion, coverage=false, 30 epoch) 启动: $(date) ===="
/home/pi/anaconda3/envs/hyper-yolo/bin/python -m ultralytics.models.yolo.detect.train \
  ...
  epochs=30 patience=10 batch=16 imgsz=1024 \
  save=True val_period=1 start_val_epoch=0 save_period=-1 \
  cache=False device=0 workers=2 \
  project=runs/coil_loss_ablation name=11_paaug_motion_v2_smoke exist_ok=True \
  ...
  2>&1 | tee /home/pi/projects/hyperyolo/runs/coil_loss_ablation/11_paaug_motion_v2_smoke.log
echo "==== D smoke test v2 完成: $(date) ===="