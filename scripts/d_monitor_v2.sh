#!/bin/bash
# D 项训练进度监控 v2：监控 4 个 _v2 run（coverage=false 修复版）
# 用法：bash scripts/d_monitor_v2.sh
set -u
...
RUNS=(
    "11_paaug_motion_v2"
    "11_paaug_reflection_v2"
    "11_paaug_occlusion_v2"
    "11_paaug_noise_v2"
)
...
echo "[$(date '+%H:%M:%S')] D v2 多 run 监控启动" > "$LOG_BASE/d_v2_monitor.log"
